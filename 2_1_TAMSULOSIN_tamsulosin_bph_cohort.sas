...
proc sql;
	...
	from output.Rx_PostStart as p
		inner join output.EarliestDate as e
		on p.patid = e.patid
...
PROC SQL;
create table bph_tam_filter AS
select * 
from output.EarliestRx_SingleDrug
...
/**************************************************************************/
	...
	FROM 
		output.drug AS med
	INNER JOIN
		codelist.tamsulosin_only AS nlcod
		ON med.ProdCodeId = nlcod.ProdCodeId
	WHERE nlcod.quantity > 0;
quit;
