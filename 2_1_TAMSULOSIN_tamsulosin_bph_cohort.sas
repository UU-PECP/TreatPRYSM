...
proc sql;
	...
	where p.exposure = e.exposure
	  and p.baseline_dt >= e.earliest_rx_date
	order by p.patid, p.baseline_dt;
quit;
...
/**************************************************************************/
	...
	SELECT med.patid, 
		   ...
		   bphcod.mg_dose
	FROM 
		rawdata.drugissue_2 AS med /*Change to include all 4 files */
	INNER JOIN
		codelist.bph_drugs_fixed AS bphcod
		ON med.ProdCodeId = bphcod.ProdCodeId;
quit;
...
proc sql;
	create table output.BphDrugAtc_2 as /* change to _2, _3, and _4 per file */
	select b.*, a.ATC
	from output.BphIndexDrug b
		left  outer join rawdata.product_aurum_atc as a
		on b.prodcodeid = a.prodcodeid;
...
proc sort data = output.BphDrugAtc_2;
by duration;
run;
...
/**************************************************************************/
	...
	SELECT med.patid, 
		   ...
		   bphcod.mg_dose
	FROM 
		rawdata.drugissue_3 AS med /*Change to include all 4 files */
	INNER JOIN
		codelist.bph_drugs_fixed AS bphcod
		ON med.ProdCodeId = bphcod.ProdCodeId;
quit;
...
proc sql;
	create table output.BphDrugAtc_3 as /* change to _2, _3, and _4 per file */
	select b.*, a.ATC
	from output.BphIndexDrug b
		left  outer join rawdata.product_aurum_atc as a
		on b.prodcodeid = a.prodcodeid;
...
proc sort data = output.BphDrugAtc_3;
by duration;
run;
...
/**************************************************************************/
	...
	SELECT med.patid, 
		   ...
		   bphcod.mg_dose
	FROM 
		rawdata.drugissue_4 AS med /*Change to include all 4 files */
	INNER JOIN
		codelist.bph_drugs_fixed AS bphcod
		ON med.ProdCodeId = bphcod.ProdCodeId;
quit;
...
proc sql;
	create table output.BphDrugAtc_4 as /* change to _2, _3, and _4 per file */
	select b.*, a.ATC
	from output.BphIndexDrug b
		left  outer join rawdata.product_aurum_atc as a
		on b.prodcodeid = a.prodcodeid;
