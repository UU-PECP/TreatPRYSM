...
libname codelist "F:\Users\Wyatt003\Tamsulosin\Codelists";
options fullstimer;
/**************************************************************************/
/* Create initial codelist from ATC codes for analgesics and NSAIDs       */
/**************************************************************************/

proc sql;
	CREATE TABLE codelist.nl_standardcare AS
	SELECT ATC, prodcodeid, productname, BNF, drugissues
...
/**************************************************************************/
/* Create mg_dose from productname in nl_standardcare codelist            */
/**************************************************************************/
/* Logic:
   - mg/Xml or mg/ml (concentration) formats -> take the mg number as-is
     (treated as per-unit strength, per Sage's decision)
   - % formulations (topical/gel) -> mg_dose left missing (not oral dosing)
...
*** Script written by Claude Sonnet 4.6 given instructions by Sage */
...
data codelist.nl_standardcare;
    ...
    if index(productname, '%') > 0 then do;
        ...
        mg_dose = input(num_str, best12.);

        if lowcase(unit_found) in ('microgram','micrograms','mcg') then mg_dose = mg_dose / 1000;
        else if lowcase(unit_found) = 'g' then mg_dose = mg_dose * 1000;
        /* if mg, leave as-is */
    end;
    else do;
        mg_dose = .;
...
proc print data=codelist.nl_standardcare (obs=40);
    var productname mg_dose;
run;

proc sql;
    select count(*) as n_total,
           sum(mg_dose is not missing) as n_populated,
           sum(mg_dose is missing) as n_missing
    from codelist.nl_standardcare;
quit;


/**************************************************************************/
/* Merge standard care file with tamsulosin to create overall nl drugs    */
/**************************************************************************/

data codelist.tamsulosin_only;
set codelist.bph_drugs_fixed ;
where drugsubstancename = "Tamsulosin";
run;

data codelist.nl_drugs_fixed;
set codelist.nl_standardcare;
drugsubstancename = "Analgesic";
run;

data codelist.nl_drugs;
set codelist.tamsulosin_only (keep = prodcodeid drugsubstancename mg_dose)
codelist.nl_drugs_fixed (keep = prodcodeid drugsubstancename mg_dose);
run;
