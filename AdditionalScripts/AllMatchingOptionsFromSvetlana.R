...
run_ps_match_imput_pipeline <- function(df, replace=F, m = 5, seed = 123, caliper = 0.2) {  
  ...
  matched <- lapply(matched,function(x)x[,c("exposure","subclass")])
...
###
alf_results_less_matched$matched[[1]][1:5,]
alf_results_less_matched$cox_fit_crude
names(alf_results_less_matched$cox_fit_crude[[1]])
...
alf_hr <- lapply( alf_results_less_matched$cox_fit_crude, tbl_regression, exponentiate = TRUE)
path_tb <- "C:\\Users\\Wyatt003\\OneDrive - Universiteit Utrecht\\Documents\\Export\\alfuzosin_pp_cox_5.docx"
alf_hr_5 <- lapply( alf_hr, function(x) save_as_docx( as_flex_table(x),path=path_tb) )
alf_hr_5 <- lapply( alf_hr, function(x) save_as_docx( as_flex_table(x),path=path_tb) )


#alf_hr <- tbl_regression(alf_results_less_matched$cox_fit_crude, exponentiate = TRUE)
...
alf_inc <- as.data.frame(alf_results_less_matched$summary_data)
